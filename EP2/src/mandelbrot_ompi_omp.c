...
{
    FILE *file;
    char *filename = "output_ompi_omp.ppm";
    char *comment = "# ";
...
void compute_mandelbrot(unsigned char **sub_image_buffer, int j)
{
    ...
    #pragma omp parallel for num_threads(NTHREADS) private(i_x,z_x,z_y,z_x_squared,z_y_squared,c_x,iteration) shared(c_y,pixel_width,escape_radius_squared,c_x_min,iteration_max,i_x_max)
